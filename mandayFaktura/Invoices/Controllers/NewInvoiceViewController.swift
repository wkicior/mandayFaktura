...
class NewInvoiceViewController: NSViewController {
    ...
    @IBAction func onSaveButtonClicked(_ sender: NSButton) {
        ...
        let invoice = Invoice(issueDate: issueDatePicker.dateValue, number: numberTextField.stringValue, sellingDate: Date(), seller: seller, buyer: buyer, items: [], paymentForm: .cash, paymentDueDate: Date())
